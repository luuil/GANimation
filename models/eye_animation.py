...
class EyeAnimation(BaseModel):
    def __init__(self, opt):
        super(EyeAnimation, self).__init__(opt)
        self._name = 'EyeAnimation'
    ...
    def _init_prefetch_inputs(self):
        self._input_rest_img = self._Tensor(self._opt.batch_size, 3, self._opt.image_size, self._opt.image_size)
        self._input_expressive_img = self._Tensor(self._opt.batch_size, 3, self._opt.image_size, self._opt.image_size)
        self._input_pose_vec = self._Tensor(self._opt.batch_size, self._opt.cond_nc)
        self._input_rest_name = None
        self._input_expressive_name = None

    def _init_losses(self):
        # define loss functions
        self._loss_func_l1 = torch.nn.L1Loss().cuda()
        # self._criterion_D_cond = torch.nn.MSELoss().cuda()

        self._loss_val_l1 = Variable(self._Tensor([0]))
    ...
    def set_input(self, input):
        self._input_rest_img.resize_(input['rest'].size()).copy_(input['rest'])
        self._input_pose_vec.resize_(input['pose'].size()).copy_(input['pose'])
        self._input_expressive_img.resize_(input['expressive'].size()).copy_(input['expressive'])
        self._input_rest_name = input['rest_name']
        self._input_expressive_name = input['expressive_name']

        if len(self._gpu_ids) > 0:
            self._input_rest_img = self._input_rest_img.cuda(self._gpu_ids[0])
            self._input_expressive_img = self._input_expressive_img.cuda(self._gpu_ids[0])
    ...
    def get_image_paths(self):
        return OrderedDict([('rest_name', self._input_rest_name)])
    ...
    def forward(self, keep_data_for_visuals=False, return_estimates=False):
        ...
        with torch.no_grad():
            ...
            fake_imgs_masked = fake_img_mask * rest_img + (1 - fake_img_mask) * fake_imgs
        ...
        if return_estimates:
            ...
            im_concat_img = np.concatenate([im_real_img, im_expressive_img,  im_fake_imgs_masked, im_fake_img_mask_norm, im_fake_imgs,
            ...
            data = OrderedDict([('rest_name', self._input_rest_name),
    ...
    def forward_one(self, rest_img: torch.Tensor, pose_vec: torch.Tensor):
        ...
        im_fake_imgs_masked = util.tensor2im(fake_imgs_masked.data)
        im_concat_img = np.concatenate([im_fake_imgs_masked, im_fake_img_mask_norm, im_fake_imgs], 1)
        imgs = {
    ...
    def optimize_parameters(self, train_generator=True, keep_data_for_visuals=False):
        if self._is_train:
            # convert tensor to variables
            self._rest_img = Variable(self._input_rest_img)
            self._expressive_img = Variable(self._input_expressive_img)
            self._pose_vec = Variable(self._input_pose_vec)
    ...
    def _forward_G(self, keep_data_for_visuals):
        # generate fake images
        fake_imgs, fake_img_mask = self._G.forward(self._rest_img, self._pose_vec)
        fake_img_mask = self._do_if_necessary_saturate_mask(fake_img_mask, saturate=self._opt.do_saturate_mask)
        fake_imgs_masked = fake_img_mask * self._rest_img + (1 - fake_img_mask) * fake_imgs

        # l1 loss
        self._loss_val_l1 = self._loss_func_l1(fake_imgs_masked, self._expressive_img)
